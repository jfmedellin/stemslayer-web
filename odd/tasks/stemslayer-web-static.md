...
- [x] T5 — Browser storage and memory design. IndexedDB schema mirroring `history.py` (tracks, stems as Blobs, profile fingerprint in the cache key as `stem_cache.cache_key` does), quota strategy and eviction (the `sweep_orphans` equivalent), stem format (WAV vs Float32 planar), chunked inference for long songs, WASM memory ceiling, and what happens on quota errors. Output: design note.
  ...
  - Done 2026-09-20: `docs/decisions/browser-storage.md`. One IndexedDB store with a unique `[sourceHash, pipelineFingerprint]` index under a Web Lock; stems as float32 WAV files in OPFS (≈101 MiB per stem per 5 min; ≈404 MiB Basic, ≈606 MiB Rock); weights in a revision-named Cache API cache with SHA-256 verification (≈166 MiB + ≈272 MiB); quota pre-flight and referential orphan sweep; soft drop-zone ceiling around 15 minutes pending S1/S2 measurements. Parity table: 34 desktop persistence rules mapped, none unmapped. Reviewed by the parent; weight sizes and runtime references updated to the T3b picks. One carried-over unknown: whether a browser `File` snapshot satisfies the ARC-02 immutable-input rule or an OPFS staging copy is needed (resolve in phase 2).
- [x] T6 — Architecture and repository decision. Decided 2026-09-19: a separate repository (different stack, CI target and release cadence; no shared runtime code). Repository `stemslayer-web` created locally on 2026-09-19 and this document moved here. Remaining: layout, toolchain (Vite + TypeScript is the working assumption), Web Worker for inference, Web Audio graph for the mixer (one source + gain node per lane, master gain, loop), `OfflineAudioContext` export, test runner, CI to GitHub Pages. Output: decision record.
  - Route: inline after T3–T5 (decision, not exploration). Checks: user confirms repository name and toolchain.
  - Done 2026-09-20: `docs/decisions/architecture.md`. Vite + React + TypeScript strict; Vitest in Node for domain/application and Vitest browser mode (Playwright, Chromium) for infrastructure; hexagonal layers (`domain`, `application` with ports, `infrastructure` adapters, `ui` atomic design); one inference Worker on onnxruntime-web (WebGPU, WASM fallback); one AudioWorklet mixer; GitHub Actions to Pages. TDD on for domain/application, runner `vitest`. Toolchain confirmed by the user 2026-09-20.
- [x] T7 — Phase-2 task list. Ordered implementation tasks sized by the ~400 authored-line planning heuristic, each with its checks, to be authorized separately after the go/no-go.
  - Route: inline. Checks: user reviews and authorizes or stops.
  - Done 2026-09-20: `docs/decisions/phase-2-plan.md`. Two spikes (S2 runtime timing and numeric diff, S1 cross-origin isolation) plus 15 tasks P1–P11 sized by the ~400-line heuristic, ~5,900 authored lines forecast, one PR per task. Awaiting the go/no-go.
...
- 2026-09-20 — T6 and T7 done. All phase-1 tasks closed. Awaiting the user's go/no-go.
- 2026-09-20 — T5 done.
